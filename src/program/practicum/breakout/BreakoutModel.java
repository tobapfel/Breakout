...
public class BreakoutModel {
	...
	/**
	 * Constructor
	 * @param rows [int] The number of brick rows
	 * @param columns [int] The number of brick columns
	 * @param lives [int] The number of lives, the user has.
	...
	public BreakoutModel(int rows, int columns, int lives){
		...
		if(columns < 1){
			throw new IllegalArgumentException("The game needs at least 1 column of bricks. You've entered: " + columns);
		}
		if(lives < 1){
	 ...
	 * @param column [int] the column of the brick
	 */
